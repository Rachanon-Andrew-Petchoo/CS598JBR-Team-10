...
def prompt_model(dataset, model_name = "deepseek-ai/deepseek-coder-6.7b-base", quantization = True):
    ...
    for case in dataset:
        ...
        def extract_code(txt: str) -> str:
            fences = re.findall(r"```(?:python)?\s*([\s\S]*?)```", txt, flags=re.IGNORECASE)
            candidate = fences[0] if fences else txt

            m = re.search(r"(?s)^\s*(def\s+[A-Za-z_]\w*\s*\(.*?\):.*)", candidate, flags=re.MULTILINE)
            if m:
                candidate = m.group(1)

            m2 = re.search(r"(?s)(def\s+[A-Za-z_]\w*\s*\(.*?\):.*?)(?:\n\s*def\s+[A-Za-z_]\w*\s*\(|\Z)", candidate)
            if m2:
        ...
        response_processed = extract_code(response)
